...
void FollowCamera::OnUpdate(SceneNode& parent) {
	glm::vec3 springarm = parent.transform.rotation * 
		glm::vec4(glm::normalize(glm::vec3(0, -1, 5)), 1);
	transform.position = parent.transform.position - springarm * 50.0f;
	m_Forward = glm::rotate(glm::mat4(1), m_Yaw, glm::vec3(0, 1, 0)) *
		glm::rotate(glm::mat4(1), m_Pitch, m_Right) * 
		glm::vec4(springarm, 0);
